...
bool COSDir::GetNextDir ( COSDir &oDir, bool bRecursive )
{
#ifdef _WIN32
  std::string realMask = "*.*";	//FIXME -- could this also be just '*' ?
#else
  ...
  if (info->namePos == 0xFFFFFFFF)
  {
    ...
    //FIXME -- just do the #ifdef'ing here?
#ifdef _WIN32
    WindowsAddFileStack(GetFullOSPath(),realMask.c_str(),bRecursive,true);
#else
    LinuxAddFileStack(GetFullOSPath(),realMask.c_str(),bRecursive,true);
#endif // _WIN32
...
bool COSDir::WindowsAddFileStack ( const char *szPathName, const char* fileMask, bool bRecursive, bool dirOnly )
{
#ifdef _WIN32
  ...
  if (hFile != -1)
  {
	...
	if (dirOnly)
	{
	  if ( (fileInfo.attrib & _A_SUBDIR ))	// we are just doing dirs, so skip files
	  {
	    info->nameList.push_back(FilePath); // store the dir
	    if (bRecursive)						// push into that dir if we are recursive.
...
bool COSDir::LinuxAddFileStack (  const char *szPathName, const char* fileMask, bool bRecursive , bool dirOnly )
{
#ifdef _WIN32
  return false;//WIN32
#else
  ...
  while ((fileInfo = readdir(directory)))
  {
    if (!((strcmp(fileInfo->d_name, ".") == 0) || (strcmp(fileInfo->d_name, "..") == 0)))
    {
      ...
      if (dirOnly)
      {
	if (S_ISDIR(statbuf.st_mode))	// we are just doing dirs, so skip files
	{
	  info->nameList.push_back(FilePath); // store the dir
	  if (bRecursive)						// push into that dir if we are recursive.
